...
rm -f "${LOCK_DIR}/${RUN_ID}.pid" "${LOCK_DIR}/${RUN_ID}.lock"
rmdir "${LOCK_DIR}/${RUN_ID}.lck" 2>/dev/null || true

# NOTE: do NOT remove ${RUN_ID}.slot here. The slot lifecycle is owned by
# scripts/run-task.sh's release_slot() trap; deleting the .slot file before
# the trap fires turns release_slot() into a no-op and leaks the slot
# directory, blocking future runs.
