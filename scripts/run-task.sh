...
if [[ "${MODE}" == "--bg" && "${RUN_TASK_BG_CHILD:-0}" != "1" ]]; then
  ...
  echo "Queued ${RUN_ID} in background (pid ${bg_pid})."
  exit 0
fi

# Inside the bg child: $! from the parent is unreliable when nohup re-execs,
# so overwrite PID_FILE with our own pid as the source of truth.
if [[ "${MODE}" == "--bg" && "${RUN_TASK_BG_CHILD:-0}" == "1" ]]; then
  echo "$$" > "${PID_FILE}"
fi
...
reap_orphan_slots() {
  # Reclaim any slot-N whose owning task is no longer alive. A slot is
  # considered orphaned when no .slot file in LOCK_DIR points at it OR the
  # owning .pid file is missing/stale. Safe to call before each acquire
  # attempt — guards against leaked slots from earlier crashed runs.
  local dir slot_id pid pid_file alive used
  for dir in "${SLOTS_DIR}"/slot-*; do
    [[ -d "${dir}" ]] || continue
    slot_id="$(basename "${dir}")"
    slot_id="${slot_id#slot-}"
    used=0
    for pid_file in "${LOCK_DIR}"/issue-*.slot; do
      [[ -f "${pid_file}" ]] || continue
      if [[ "$(<"${pid_file}")" == "${slot_id}" ]]; then
        local owner_run owner_pid_file owner_pid
        owner_run="$(basename "${pid_file}" .slot)"
        owner_pid_file="${LOCK_DIR}/${owner_run}.pid"
        ...
        # .slot exists but owner pid is dead/missing → stale. Drop it.
        rm -f "${pid_file}"
      fi
    done
    if [[ ${used} -eq 0 ]]; then
...
release_slot() {
  ...
  if [[ -f "${slot_file}" ]]; then
    ...
    # Fallback when the .slot lookup file was removed by an earlier cleanup
    # step — the in-memory SLOT_ID set during acquire_slot is the source of
    # truth.
    slot_id="${SLOT_ID}"
  fi
  if [[ -n "${slot_id}" ]]; then
...
# Mark the GitHub issue as actually in-progress only once the container is
# up and we are about to do real work. Earlier (in start.sh / slot-acquire)
# would lie about state if the launcher crashed or stalled waiting for a
# slot.
gh issue edit "${ISSUE_NUMBER}" --add-label "agent-in-progress" --remove-label "agent-ready" >/dev/null 2>&1 || true
